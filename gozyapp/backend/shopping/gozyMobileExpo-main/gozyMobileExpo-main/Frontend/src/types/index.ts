...
export type CabTripMode = 'airport' | 'outstation' | 'hourly' | 'rail';

export type CabRideKind = 'one-way' | 'round-trip';

export type CabSearchParams = {
  tripMode: CabTripMode;
  rideKind: CabRideKind;
  pickupLabel: string;
  dropLabel: string;
...
export type CabOffer = {
  ...
  vehicleName: string;
  similarLabel: string;
  vehicleType: 'Hatchback' | 'Sedan' | 'SUV' | 'Premium';
  energyType: 'Petrol' | 'Diesel' | 'Electric';
  seats: number;
  bags: number;
  ac: boolean;
  rating: number;
  ...
  inclusions: string[];
  flightTrackingRecommended?: boolean;
};

export type CabRecentSearch = {
  id: string;
  from: string;
  to: string;
  dateLabel: string;
...
export type CabSearchResult = {
  ...
  offers: CabOffer[];
  coupon: CabCoupon;
  policyGroups: CabPolicyGroup[];
};

export type CabTraveler = {
  fullName: string;
  mobile: string;
  email: string;
  gender: 'Male' | 'Female' | 'Other' | '';
  usePickupAsBilling: boolean;
  flightNumber: string;
};
