...
import { cabRecentSearchesDefault, defaultCabSearch, defaultCabTraveler } from '@/src/lib/cab-data';
...
type SuperAppState = {
  ...
  cabSearch: CabSearchParams;
  cabResults: CabSearchResult | null;
  selectedCabOffer: CabOffer | null;
  cabTraveler: CabTraveler;
  cabRecentSearches: CabRecentSearch[];
  ...
  setCabSearch: (search: CabSearchParams) => void;
  setCabResults: (results: CabSearchResult) => void;
  selectCabOffer: (offer: CabOffer) => void;
  setCabTravelerField: (field: keyof CabTraveler, value: string | boolean) => void;
  addCabRecentSearch: (search: CabRecentSearch) => void;
  resetCabFlow: () => void;
};
...
export const useSuperAppStore = create<SuperAppState>((set) => ({
  ...
  cabSearch: defaultCabSearch,
  cabResults: null,
  selectedCabOffer: null,
  cabTraveler: defaultCabTraveler,
  cabRecentSearches: cabRecentSearchesDefault,
  ...
  setCabResults: (cabResults) =>
    set(() => ({
      cabResults,
      selectedCabOffer: cabResults.offers[0] ?? null,
    })),
  selectCabOffer: (selectedCabOffer) =>
    set(() => ({
      selectedCabOffer,
    })),
  setCabTravelerField: (field, value) =>
    set((state) => ({
      cabTraveler: {
        ...state.cabTraveler,
        [field]: value,
  ...
  addCabRecentSearch: (search) =>
    set((state) => ({
      cabRecentSearches: [search, ...state.cabRecentSearches.filter((item) => item.id !== search.id)].slice(
        0,
        6,
      ),
    })),
