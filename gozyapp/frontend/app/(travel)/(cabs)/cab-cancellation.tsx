...
export default function CabCancellationScreen() {
  ...
  const finalPriceStr = params.price || '₹ 9,157';

  const totalFare = parseFloat(finalPriceStr.replace(/[^0-9.]/g, '')) || 9157;
  const cabCancellationFee = 1500;
  const platformFee = 100;
  const totalCancellationFee = cabCancellationFee + platformFee;
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFF' }}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statusContainer}>
          ...
          <Text style={styles.statusTitle}>Cab Cancelled</Text>
          <Text style={styles.statusSub}>Your cab has been successfully cancelled.</Text>
        </View>

        {/* Cab Info */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Ride Details</Text>
          <Text style={styles.routeText}>{pickup} to {drop}</Text>
          <Text style={styles.flightDate}>23 Apr 2024</Text>
          <Text style={styles.pnrText}>BOOKING ID: CAB982Z</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>Cab Cancellation Fee</Text>
            <Text style={styles.breakdownValueNegative}>-₹ {cabCancellationFee.toLocaleString('en-IN')}</Text>
          </View>
