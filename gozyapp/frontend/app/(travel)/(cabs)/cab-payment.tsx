...
export default function CabPaymentScreen() {
  ...
  const finalPrice = params.price || '₹ 9,157';
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          bookExperience({
            ...
            rating: 5,
            reviews: 100,
            price: parseFloat(finalPrice.replace(/[^0-9.]/g, '')) || 9157,
          ...
          router.replace({
            pathname: '/(travel)/(cabs)/cab-confirmation',
            params: { type: params.type, price: finalPrice, pickup, drop, vehicle }
          });
        }, 1500);
      }, 3000);
    }, 2000);
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={styles.headerSubtitle}>{pickup} → {drop}</Text>
          </View>
        </View>
        <Text style={styles.headerPrice}>{finalPrice}</Text>
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryCard}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
            ...
            <Text style={styles.summaryTitle}>{vehicle} Ride</Text>
          </View>
          <Text style={styles.summarySub}>Pickup: {pickup}</Text>
          <Text style={styles.summarySub}>Drop: {drop}</Text>
        </View>
           ...
           {expandedSection === 'UPI' && (
             <View style={styles.accordionContent}>
                ...
                <Pressable style={styles.payBtn} onPress={handlePayment} disabled={isProcessing}>
                  {isProcessing ? <ActivityIndicator color="#FFF" /> : <Text style={styles.payBtnText}>PAY NOW VIA UPI</Text>}
                </Pressable>
             </View>
           )}
        </View>
        ...
        <View style={styles.accordionContainer}>
           ...
           {expandedSection === 'CARDS' && (
             <View style={styles.accordionContent}>
                ...
                <Pressable style={styles.payBtn} onPress={handlePayment} disabled={isProcessing}>
                  {isProcessing ? <ActivityIndicator color="#FFF" /> : <Text style={styles.payBtnText}>PAY {finalPrice}</Text>}
                </Pressable>
             </View>
           )}
        </View>
             ...
             <View style={styles.accordionContent}>
                ...
                <View style={styles.bankList}>
                   {popularBanks.map(bank => (
                     <Pressable key={bank.id} onPress={() => { setSelectedBank(bank.id); handlePayment(); }} style={styles.bankRow}>
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
