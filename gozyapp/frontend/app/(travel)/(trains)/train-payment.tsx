...
export default function TrainPaymentScreen() {
  ...
  useEffect(() => {
    if (!reviewBookingDraft) {
      router.replace('/train-results');
    }
  }, [reviewBookingDraft]);

  if (!reviewBookingDraft) return null;
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={{backgroundColor: '#FFF', paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#E5E5EA'}}>
          <View>
             ...
             <Text style={{fontSize: 10, color: '#8E8E93'}}>{booking.train.departureStation} - {booking.train.arrivalStation}</Text>
          </View>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
             <Text style={{fontSize: 13, fontWeight: '800', color: '#333', marginRight: 4}}>₹ {booking.totalPrice.toFixed(2)}</Text>
             ...
             <View style={{padding: 16}}>
                ...
                <View style={{backgroundColor: '#F0FDF4', padding: 12, borderRadius: 8, marginBottom: 16}}>
                   <View style={{flexDirection: 'row', marginBottom: 8}}>
                     ...
                     <Text style={{fontSize: 10, color: '#16A34A', marginLeft: 6}}>Link for payment of ₹ {booking.totalPrice.toFixed(2)} will be valid for 15 minutes</Text>
                   </View>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={styles.headerSubtitle}>{booking.train.departureStation} → {booking.train.arrivalStation}</Text>
          </View>
        </View>
        <Text style={styles.headerPrice}>₹ {booking.totalPrice.toFixed(2)}</Text>
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryCard}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
            <MaterialCommunityIcons name="train" size={24} color="#0084FF" />
            <Text style={styles.summaryTitle}>{booking.train.name} (#{booking.train.number})</Text>
          </View>
          <Text style={styles.summarySub}>Class: {booking.slot.className} | Date: {booking.train.departureDateLabel}</Text>
          <Text style={styles.summaryTraveller}>{booking.passengers.length} Traveller(s)</Text>
        </View>
        ...
        <View style={styles.donationCard}>
          <View style={styles.donationTop}>
            ...
            <View style={{flex: 1, marginLeft: 12}}>
              ...
              <Text style={styles.donationSub}>& earn ₹100 myCash <Text onPress={() => alert("Feature coming soon!")} style={{color: '#0084FF'}}>T&C</Text></Text>
            </View>
        ...
        <View style={{marginTop: 24, paddingHorizontal: 16, paddingBottom: 40}}>
           <Text style={{fontSize: 10, color: '#8E8E93', lineHeight: 14}}>
             By proceeding to pay, you understand and agree with the <Text onPress={() => alert("Feature coming soon!")} style={{color: '#0084FF'}}>Terms of Service</Text>, <Text onPress={() => alert("Feature coming soon!")} style={{color: '#0084FF'}}>Privacy Policy</Text> and <Text onPress={() => alert("Feature coming soon!")} style={{color: '#0084FF'}}>User Agreement</Text> of MakeMyTrip.
           </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 10.5, color: '#8E8E93', marginTop: 2, maxWidth: 200 },
  headerPrice: { fontSize: 18, fontWeight: '900', color: '#333' },

  content: { padding: 16 },

  summaryCard: { backgroundColor: '#FFF', borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#E5E5EA' },
