...
import { StyleSheet, Text, View, SafeAreaView, Pressable, ScrollView, TextInput, ActivityIndicator, Modal, Image as RNImage } from 'react-native';
import { router, useLocalSearchParams } from 'expo-router';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { MaterialCommunityIcons } from '@expo/vector-icons';
import { Image } from 'expo-image';

import { usePassengerStore } from '@/src/store/bus-passenger-store';
...
export default function BusPaymentScreen() {
  ...
  const displayPrice = parseInt(params.grandTotal || '1050', 10);
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          router.push({
            pathname: '/(bus-module)/booking-confirmation',
            params: params,
          });
        }, 1500);
      ...
      <View style={styles.safeArea}>
        ...
        <View style={{backgroundColor: '#FFF', paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#E5E5EA'}}>
          <View>
             ...
             <Text style={{fontSize: 10, color: '#8E8E93'}}>{params.fromCity} - {params.toCity}</Text>
          </View>
          ...
          {paymentMode === 'CARDS' && (
             <View style={{padding: 16}}>
                <View style={{backgroundColor: '#FFF2E6', padding: 12, borderRadius: 8, flexDirection: 'row', marginBottom: 20}}>
                   ...
                   <Text style={{fontSize: 10, color: '#D97706', marginLeft: 8, flex: 1}}>Please ensure your card is enabled for online transactions. <Text onPress={() => alert("Feature coming soon!")} style={{color: '#0084FF', fontWeight: '800'}}>Know more</Text></Text>
                </View>
    ...
    <View style={styles.safeArea}>
      ...
      <View style={[styles.header, { paddingTop: Math.max(insets.top, 16) }]}>
        ...
        <View style={styles.timerBox}>
          <MaterialCommunityIcons name="clock-outline" size={18} color="#00A699" />
          <Text style={styles.timerText}>06:47</Text>
        </View>
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.totalDueContainer}>
          <View style={styles.totalDueRow}>
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Text style={styles.totalDueAmount}>₹ {displayPrice.toLocaleString('en-IN')}</Text>
          ...
          <View style={styles.journeyInfoCard}>
            ...
            <View style={styles.journeyDetailsBox}>
              <Text style={styles.journeyRoute}>{params.fromCity || 'Bangalore'} <MaterialCommunityIcons name="arrow-right" size={12} color="#000" /> {params.toCity || 'Ahmedabad'}</Text>
              <Text style={styles.journeySubText}>
                Starting: {params.date ? new Date(params.date).toDateString() : "Fri, 24 Apr'26"} | {params.boardingTime || '4:00 PM'}
              </Text>
              <Text style={styles.journeyOperator}>{params.operator || 'Jain travels regd'}</Text>
            </View>
          </View>

          <View style={styles.passengerInfoRow}>
            <Text style={styles.passengerInfoText}>Passenger (1) 21yrs , AC Sleeper (2+1)</Text>
          </View>
        </View>

        {/* Additional Discounts Block */}
        <View style={styles.discountsCard}>
          <View style={{flex: 1}}>
            <Text style={styles.discountsTitle}>Additional discounts and saved payment options</Text>
            <Text style={styles.discountsSub}>Login to access saved payments and discounts!</Text>
          </View>
        ...
        <View style={styles.giftCardRow}>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            ...
            <Text style={styles.giftCardTitle}>Gift Cards</Text>
          </View>
          <MaterialCommunityIcons name="chevron-down" size={20} color="#0084FF" />
        </View>
        ...
        <View style={{backgroundColor: '#FFF', borderRadius: 12, overflow: 'hidden'}}>
          ...
          <Pressable style={styles.paymentOptionListRow} onPress={() => setPaymentMode('NETBANK')}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <MaterialCommunityIcons name="bank-transfer-out" size={24} color="#0084FF" />
              <View style={{marginLeft: 16}}>
                <Text style={styles.paymentOptionListTitle}>Net Banking <View style={{backgroundColor: '#00A699', paddingHorizontal: 4, borderRadius: 10}}><Text style={{color: '#FFF', fontSize: 8, fontWeight: '800'}}>Fingerprint/Face ID</Text></View></Text>
                <Text style={styles.paymentOptionListSub}>40+ Banks Available</Text>
              </View>
            </View>
            <MaterialCommunityIcons name="chevron-right" size={24} color="#0084FF" />
          </Pressable>
          ...
          <Pressable style={styles.paymentOptionListRow} onPress={() => setPaymentMode('EMI')}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <MaterialCommunityIcons name="cash-multiple" size={24} color="#0084FF" />
              <View style={{marginLeft: 16}}>
                <Text style={styles.paymentOptionListTitle}>EMI <View style={{backgroundColor: '#00A699', paddingHorizontal: 4, borderRadius: 10}}><Text style={{color: '#FFF', fontSize: 8, fontWeight: '800'}}>NO COST EMI</Text></View></Text>
          ...
          <Pressable style={styles.paymentOptionListRow} onPress={() => setPaymentMode('WALLET')}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <View style={{marginLeft: 16}}>
                ...
                <Text style={styles.paymentOptionListSub}>MMT Gift cards, Amazon Pay</Text>
              </View>
            </View>
            <MaterialCommunityIcons name="chevron-right" size={24} color="#0084FF" />
          </Pressable>
          ...
          <Pressable style={styles.paymentOptionListRow} onPress={() => setPaymentMode('UPI')}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Image source={{uri: 'https://cdn.iconscout.com/icon/free/png-256/free-google-pay-2036034-1713217.png'}} style={{width: 24, height: 24}} contentFit="contain" />
              <View style={{marginLeft: 16}}>
                <Text style={styles.paymentOptionListTitle}>GooglePay</Text>
                <Text style={styles.paymentOptionListSub}>Pay with GooglePay</Text>
              </View>
            </View>
            <MaterialCommunityIcons name="chevron-down" size={24} color="#0084FF" />
          </Pressable>
          
          {/* PhonePe */}
          <Pressable style={[styles.paymentOptionListRow, { borderBottomWidth: 0 }]} onPress={() => setPaymentMode('UPI')}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Image source={{uri: 'https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/phonepe-logo-icon.png'}} style={{width: 24, height: 24}} contentFit="contain" />
              <View style={{marginLeft: 16}}>
                <Text style={styles.paymentOptionListTitle}>PhonePe</Text>
                <Text style={styles.paymentOptionListSub}>Pay with PhonePe</Text>
              </View>
            </View>
            <MaterialCommunityIcons name="chevron-down" size={24} color="#0084FF" />
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  timerText: { fontSize: 14, fontWeight: '800', color: '#00A699' },

  content: { flex: 1, paddingHorizontal: 16 },

  totalDueContainer: { backgroundColor: '#FFF', padding: 16, borderBottomWidth: 1, borderBottomColor: '#E5E5EA', marginHorizontal: -16, marginBottom: 16 },
  totalDueRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  totalDueLabel: { fontSize: 13, fontWeight: '800', color: '#333' },
  totalDueAmount: { fontSize: 18, fontWeight: '800', color: '#333', marginRight: 8 },
  chevronCircle: { width: 20, height: 20, borderRadius: 10, borderWidth: 1, borderColor: '#E5E5EA', alignItems: 'center', justifyContent: 'center' },

  journeyInfoCard: { flexDirection: 'row', gap: 12, paddingVertical: 12, borderTopWidth: 1, borderTopColor: '#F2F4F7' },
  busIconBox: { width: 40, height: 40, borderRadius: 8, backgroundColor: '#0084FF', alignItems: 'center', justifyContent: 'center' },
  journeyDetailsBox: { flex: 1, gap: 4 },
  journeyRoute: { fontSize: 14, fontWeight: '800', color: '#333' },
  journeySubText: { fontSize: 11, color: '#666' },
  journeyOperator: { fontSize: 12, color: '#666' },

  passengerInfoRow: { borderTopWidth: 1, borderTopColor: '#F2F4F7', paddingTop: 12 },
  passengerInfoText: { fontSize: 11, color: '#666', fontWeight: '500' },

  discountsCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', borderRadius: 8, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#E5E5EA' },
  discountsTitle: { fontSize: 13, fontWeight: '800', color: '#333' },
  discountsSub: { fontSize: 11, color: '#8E8E93', marginTop: 4 },
  loginBtn: { backgroundColor: '#0084FF', borderRadius: 16, paddingHorizontal: 16, paddingVertical: 8 },
  loginBtnText: { color: '#FFFFFF', fontSize: 12, fontWeight: '800' },

  giftCardRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFF', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 12, borderWidth: 1, borderColor: '#E5E5EA', marginBottom: 24 },
  giftCardTitle: { fontSize: 14, fontWeight: '800', color: '#333', marginLeft: 8 },

  sectionHeading: { fontSize: 18, fontWeight: '900', color: '#333', marginBottom: 12 },
