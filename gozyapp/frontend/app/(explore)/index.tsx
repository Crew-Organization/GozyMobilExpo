...
//   advTitle: { fontSize: 18, fontWeight: '900', color: '#0F172A', textAlign: 'center', marginBottom: 8 },
...
//   planTripBtnText: { color: '#111827', fontSize: 13, fontWeight: '800' },
...
//   aiConciergeText: { fontSize: 13, fontWeight: '900', color: '#0F172A', letterSpacing: 0.5 },
...
export default function SuperAppDashboard() {
  ...
  const handleFeaturePress = (id?: string) => {
    ...
    else if (id === 'hotels' || id === 'hourly') router.push('/(hotels)/hotels');
    else if (id === 'cabs') router.push('/(travel)/(cabs)/cab');
    else if (id === 'flights') router.push('/(travel)/(flights)/travel');
    else if (id === 'shopping') router.push('/shopping');
    else if (id === 'movies' || id === 'entertainment') router.push('/(movies)/movies');
    else if (id === 'bus') router.push('/(travel)/(buses)/busbooking');
    else if (id === 'trains') router.push('/(travel)/(trains)/train');
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.header, { paddingTop: Math.max(insets.top, 16) + 16 }]}>
          <View style={[styles.headerLeft, { flexDirection: 'row', alignItems: 'center', gap: 12 }]}>
        ...
        <View style={styles.mainGridCard}>
            ...
            {visibleModules.map((item) => (
              <Pressable key={item.id} style={styles.unifiedModule} onPress={() => handleFeaturePress(item.id)}>
                <View style={styles.unifiedIconContainer}>
                  ...
                  <MaterialCommunityIcons name={item.icon as any} size={26} color="#0084FF" />
                </View>
                <Text style={styles.unifiedModuleLabel} numberOfLines={2}>{item.label}</Text>
              </Pressable>
            ))}
          ...
          <Pressable style={styles.chevronWrapper} onPress={() => setIsExpanded(!isExpanded)}>
            <MaterialCommunityIcons name={isExpanded ? 'chevron-up' : 'chevron-down'} size={24} color="#0084FF" />
          </Pressable>
        </View>
        ...
        <View style={styles.hotelsGrid}>
          <Pressable style={styles.hotelLarge} onPress={() => router.push('/(hotels)/hotels')}>
          ...
          <View style={styles.hotelRightCol}>
            {hotelStores.slice(1).map((h) => (
              <Pressable key={h.id} style={styles.hotelSmall} onPress={() => router.push('/(hotels)/hotels')}>
        ...
        {/* ── SHOPPING SECTION ── */}
        <View style={[styles.sectionHeader, { marginTop: 28 }]}>
          <Text style={styles.sectionTitle}>Trending Fashion & Tech</Text>
          <Pressable style={styles.viewAllBtn} onPress={() => router.push('/shopping')}>
            <Text style={styles.viewAllText}>View All</Text>
            <MaterialCommunityIcons name="chevron-right-circle" size={18} color="#0084FF" />
          </Pressable>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.shoppingSectionRow}>
          {products.slice(0, 5).map((product) => (
            <Pressable
              key={product.id}
              style={styles.shoppingProductCard}
              onPress={() => {
                setSelectedProduct(product.id);
                router.push('/product-detail');
              }}
            >
              <Image source={{ uri: product.image }} style={styles.shoppingProductImage} contentFit="cover" />
              {product.badge && (
                <View style={styles.shoppingProductBadge}>
                  <Text style={styles.shoppingProductBadgeText}>{product.badge}</Text>
                </View>
              )}
              <View style={styles.shoppingProductInfo}>
                <Text style={styles.shoppingProductBrand} numberOfLines={1}>{product.brand}</Text>
                <Text style={styles.shoppingProductName} numberOfLines={1}>{product.name}</Text>
                <View style={styles.shoppingProductFooter}>
                  <Text style={styles.shoppingProductPrice}>Rs {product.price.toLocaleString('en-IN')}</Text>
                  <View style={styles.shoppingProductRating}>
                    <MaterialCommunityIcons name="star" size={12} color="#F5C518" />
                    <Text style={styles.shoppingProductRatingText}>{product.rating.toFixed(1)}</Text>
                  </View>
                </View>
              </View>
            </Pressable>
        ...
        {/* ── MOVIES SECTION ── */}
        <View style={[styles.sectionHeader, { marginTop: 28 }]}>
          <Text style={styles.sectionTitle}>Movies & Live Events</Text>
          <Pressable style={styles.viewAllBtn} onPress={() => router.push('/(movies)/movies')}>
            <Text style={styles.viewAllText}>View All</Text>
            <MaterialCommunityIcons name="chevron-right-circle" size={18} color="#0084FF" />
          </Pressable>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.moviesSectionRow}>
          {events.slice(0, 5).map((event) => (
            <Pressable
              key={event.id}
              style={styles.movieEventCard}
              onPress={() => router.push('/(movies)/movies')}
            >
              <Image source={{ uri: event.image }} style={styles.movieEventImage} contentFit="cover" />
              <LinearGradient colors={['transparent', 'rgba(0,0,0,0.85)']} style={StyleSheet.absoluteFillObject} />
              <View style={styles.movieEventBadge}>
                <Text style={styles.movieEventBadgeText}>{event.genre}</Text>
              </View>
              <View style={styles.movieEventInfo}>
                <Text style={styles.movieEventTitle} numberOfLines={1}>{event.title}</Text>
                <View style={styles.movieEventFooter}>
                  <Text style={styles.movieEventVenue} numberOfLines={1}>{event.venue.split(':')[0]}</Text>
                  <Text style={styles.movieEventPrice}>from ₹{event.price}</Text>
                </View>
              </View>
            </Pressable>
          ))}
...
const styles = StyleSheet.create({
  ...
  // ── HEADER ──
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingBottom: 16, backgroundColor: '#FFFFFF', borderBottomLeftRadius: 24, borderBottomRightRadius: 24, zIndex: 10, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 8, elevation: 4 },
  headerLeft: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  headerTitle: { fontSize: 18, fontWeight: '900', color: '#0F172A' },
  headerRight: { flexDirection: 'row', alignItems: 'center', gap: 16 },
  headerRightLogos: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  myCashText: { fontSize: 18, fontWeight: '800', color: '#000', fontStyle: 'italic', letterSpacing: -0.5 },
  myBizLogo: { backgroundColor: '#E53E3E', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  myBizText: { color: '#FFF', fontSize: 13, fontWeight: '900', fontStyle: 'italic' },
  headerActionBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#E0F2FE', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
  headerActionBtnText: { fontSize: 12, fontWeight: '800', color: '#0369A1' },
  headerAction: { alignItems: 'center', justifyContent: 'center', position: 'relative' },
  notifDot: { position: 'absolute', top: -2, right: 2, width: 8, height: 8, borderRadius: 4, backgroundColor: '#EF4444', borderWidth: 1.5, borderColor: '#FFFFFF' },
  ...
  offerTitle: { color: '#FFF', fontSize: 18, fontWeight: '900', marginBottom: 2 },
  ...
  airlineName: { color: '#FFF', fontSize: 10.5, fontWeight: '800', textAlign: 'center', zIndex: 2, lineHeight: 14, textShadowColor: 'rgba(0,0,0,0.8)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
  ...
  hotelTitleLarge: { color: '#FFF', fontSize: 13, fontWeight: '900', lineHeight: 18 },
  ...
  discoverLabel: { fontSize: 10.5, fontWeight: '700', color: '#334155', textAlign: 'center', lineHeight: 15 },
  ...
  hostingText: { color: '#FFF', fontSize: 13, fontWeight: '800', flex: 1 },
  ...
  pOn: { color: '#FFF', fontSize: 10.5, fontWeight: '400', marginTop: 2 },
  pMmtRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 },
  pMake: { color: '#FFF', fontSize: 13, fontWeight: '300', fontStyle: 'italic' },
  ...
  orderBtnText: { color: '#FFFFFF', fontSize: 10.5, fontWeight: '900', letterSpacing: 1 },
  ...
  sponsoredTitle: { fontSize: 10.5, fontWeight: '700', color: '#1E293B', lineHeight: 15 },
  ...
  w2gMetaTitle: { fontSize: 10.5, fontWeight: '700', color: '#1E293B', lineHeight: 14 },
  ...
  w2gHotelTitle: { color: '#FFF', fontSize: 10.5, fontWeight: '800', lineHeight: 14 },
  ...
  viewMoreText: { color: '#FFF', fontSize: 13, fontWeight: '900', letterSpacing: 1 },
  ...
  shoppingProductRating: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 2,
  },
  shoppingProductRatingText: {
    fontSize: 9,
    fontWeight: '800',
    color: '#0F172A',
  },
